...
static _Bool mpu6050AccRead(int16_t *accData);
static void mpu6050GyroInit(void);
static _Bool mpu6050GyroRead(int16_t *gyroADC);
...
# 232 "./src/main/drivers/accgyro_mpu6050.c"
...
static _Bool mpu6050AccRead(int16_t *accData)
{
    ...
    _Bool ack = i2cRead(0x68, 0x3B, 6, buf);
    if (!ack) {
        return 0;
    }
...
static void mpu6050GyroInit(void)
{
    ...
    _Bool ack;
    ack = i2cWrite(0x68, 0x6B, 0x80);
    delay(100);
    ack = i2cWrite(0x68, 0x19, 0x00);
    ack = i2cWrite(0x68, 0x6B, 0x03);
    delay(15);
    ack = i2cWrite(0x68, 0x1A, mpuLowPassFilter);
    ack = i2cWrite(0x68, 0x1B, INV_FSR_2000DPS << 3);



    ack = i2cWrite(0x68, 0x1C, INV_FSR_8G << 3);

    ack = i2cWrite(0x68, 0x37,
            0 << 7 | 0 << 6 | 0 << 5 | 0 << 4 | 0 << 3 | 0 << 2 | 1 << 1 | 0 << 0);


    ack = i2cWrite(0x68, 0x38, (1 << 0));
...
static _Bool mpu6050GyroRead(int16_t *gyroADC)
{
    ...
    _Bool ack = i2cRead(0x68, 0x43, 6, buf);
    if (!ack) {
        return 0;
    }
